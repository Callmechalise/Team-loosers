...
import { 
  Users, HeartPulse, Siren, Watch, Ambulance, PersonStanding, 
  Heart, Thermometer, Activity, Battery, ArrowRight, Check, 
  Pill, ClipboardList, User, FileBarChart, Phone, Bell, LayoutDashboard 
} from 'lucide-react';
...
const getAvatarUrl = (name: string, size: number = 64) => {
  return `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=6366f1&color=fff&size=${size}&bold=true`;
};

// Helper function to get image URL with fallback
const getImageUrl = (url: string, name: string, size: number = 64) => {
  if (!url || url.includes('placeholder') || url.includes('pexels')) {
    return getAvatarUrl(name, size);
  }
  return url;
};

// Suppress unique key warnings temporarily (optional - remove once fixed)
...
export default function DashboardContent() {
  const { residents, alerts, activities, notifications, isConnected, lastSync, loading } = useIoTData();
  ...
  // Show empty state if no residents
  if (!residents.length) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          <p className="text-gray-600 dark:text-gray-400">No residents found. Please check your backend connection.</p>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Dashboard"
        description={`Welcome back, ${CARETAKER_PROFILE?.name?.split(' ')[0] || 'User'} — here's your facility overview`}
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div key="chart-heart-rate">
          ...
          >
            {heartRateData.length > 0 ? (
              <TrendAreaChart 
                data={heartRateData} 
                dataKey={Object.keys(heartRateData[0] || { time: '' }).filter((k) => k !== 'time')[0] || 'time'} 
                color="#EF4444" 
                xKey="time" 
                unit=" BPM" 
                height={220} 
              ...
              <div className="flex h-[220px] items-center justify-center text-gray-400">No data available</div>
            )}
          </ChartCard>
        </div>

        <div key="chart-blood-pressure">
          <ChartCard
            ...
            {bpData.length > 0 ? (
              <MultiLineChart
                ...
                lines={[
                  ...
                  { key: 'diastolic', color: '#DC2626', name: 'Diastolic' },
                ]}
                height={220}
              />
            ) : (
        ...
        <div key="chart-emergency-alerts">
          <ChartCard
            ...
            {recentAlerts.length > 0 ? (
              recentAlerts.map((alert, idx) => {
                ...
                return (
                  ...
                  >
                    <img 
                      src={getImageUrl(alert.photo, alert.residentName, 36)} 
          ...
          <div className="space-y-2">
            {liveDevices.length > 0 ? (
              liveDevices.map((r, idx) => (
                ...
                >
                  <img 
                    src={getImageUrl(r.photo, r.name, 32)} 
                    alt={r.name} 
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">Room {r.room} · {r.device?.model || 'Unknown'}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <SignalIndicator signal={r.vitals?.signal || 0} />
                    <BatteryIndicator battery={r.vitals?.battery || 0} />
                  </div>
                </motion.div>
              ))
            ) : (
